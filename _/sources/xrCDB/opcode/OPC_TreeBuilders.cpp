...
bool AABBTreeOfTrianglesBuilder::ComputeGlobalBox(const udword* primitives, udword nb_prims, AABB& global_box) const
{
	...
	// Initialize global box
	icePoint Min(flt_max, flt_max, flt_max);
	icePoint Max(flt_min, flt_min, flt_min);

	// Loop through triangles
	for (udword i = 0; i < nb_prims; i++)
	{
		// Get current triangle-vertices
		const icePoint& p0 = mVerts[mTriList[primitives[i]].mVRef[0]];
		const icePoint& p1 = mVerts[mTriList[primitives[i]].mVRef[1]];
		const icePoint& p2 = mVerts[mTriList[primitives[i]].mVRef[2]];
		// Update global box
