...
public:
	...
	inline void Scale(const icePoint& p)
	{
		m[0][0] *= p.x;
		m[0][1] *= p.x;
		m[0][2] *= p.x;
	...
	inline void Copy(const Matrix3x3& source)
	{
	...
	inline void SkewSymmetric(const icePoint& a)
	{
		m[0][0] = 0.0f;
		m[0][1] = -a.z;
		m[0][2] = a.y;
	...
	*/
	...
	inline icePoint operator*(const icePoint& v) const { return icePoint(ROW[0] | v, ROW[1] | v, ROW[2] | v); }
	...
	inline const icePoint* operator[](int nRow) const { return (const icePoint*)&m[nRow][0]; }
	inline icePoint* operator[](int nRow) { return (icePoint*)&m[nRow][0]; }
