...
public:
	 ...
	 */
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void SetMinMax(const icePoint& min, const icePoint& max)
	{
		mMin = min;
		mMax = max;
	}
	 ...
	 */
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void SetCenterExtents(const icePoint& c, const icePoint& e)
	{
		mMin = c - e;
		mMax = c + e;
	}
	...
	void SetEmpty()
	{
		icePoint p(flt_min, flt_min, flt_min);
		mMin = -p;
		mMax = p;
	}
	...
	inline void GetMin(icePoint& min) const { min = mMin; }
	//! Get max point of the box
	inline void GetMax(icePoint& max) const { max = mMax; }

	//! Get component of the box's min point along a given axis
	inline float GetMin(udword axis) const { return ((const float*)mMin)[axis]; }
	//! Get component of the box's max point along a given axis
	inline float GetMax(udword axis) const { return ((const float*)mMax)[axis]; }

	//! Get box center
	inline void GetCenter(icePoint& center) const { center = (mMax + mMin) * 0.5f; }
	//! Get box extents
	inline void GetExtents(icePoint& extents) const { extents = (mMax - mMin) * 0.5f; }

	//! Get component of the box's center along a given axis
	inline float GetCenter(udword axis) const
	{
		return (((const float*)mMax)[axis] + ((const float*)mMin)[axis]) * 0.5f;
	}

	//! Get component of the box's extents along a given axis
	inline float GetExtents(udword axis) const
	{
		return (((const float*)mMax)[axis] - ((const float*)mMin)[axis]) * 0.5f;
	}

	//! Get box diagonal
	inline void GetDiagonal(icePoint& diagonal) const { diagonal = mMax - mMin; }
	 ...
	 */
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	inline bool Intersect(const AABB& a, udword axis) const
	{
	...
	inline AABB& operator+=(const icePoint& trans)
	{
		mMin += trans;
		mMax += trans;
		return *this;
 ...
 */
		...
		//! Get min point of the box
		inline			void		GetMin(icePoint& min)						const		{ min = mCenter - mExtents;					}
		//! Get max point of the box
		inline			void		GetMax(icePoint& max)						const		{ max = mCenter + mExtents;					}
		...
		//! Get box center
		inline			void		GetCenter(icePoint& center)				const		{ center = mCenter;							}
		//! Get box extents
		inline			void		GetExtents(icePoint& extents)				const		{ extents = mExtents;						}

		//! Get component of the box's center along a given axis
		inline			float		GetCenter(udword axis)					const		{ return mCenter[axis];						}
		//! Get component of the box's extents along a given axis
		inline			float		GetExtents(udword axis)					const		{ return mExtents[axis];					}

		//! Get box diagonal
		inline			void		GetDiagonal(icePoint& diagonal)			const		{ diagonal = mExtents * 2.0f;				}
 ...
 */
		...
		//! Operator for AABB *= float. Scales the extents, keeps same center.
		inline			AABB&		operator*=(float s)		{ mExtents*=s;	return *this;	}
...
inline void ComputeMinMax(const icePoint& p, icePoint& min, icePoint& max)
{
	if (p.x > max.x) max.x = p.x;
	if (p.x < min.x) min.x = p.x;
...
inline void ComputeAABB(AABB& aabb, const icePoint* list, udword nbpts)
{
	if (list)
	{
		icePoint Maxi(flt_min, flt_min, flt_min);
		icePoint Mini(flt_max, flt_max, flt_max);
		while (nbpts--)
		{
