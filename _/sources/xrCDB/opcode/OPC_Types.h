...
typedef int ICE_Dummy_bool[(sizeof(bool) == 1) * 2 - 1]; // ...otherwise things might fail with VC++ 4.2 !
typedef int ICE_Dummy_ubyte[(sizeof(ubyte) == 1) * 2 - 1];
typedef int ICE_Dummy_sbyte[(sizeof(sbyte) == 1) * 2 - 1];
typedef int ICE_Dummy_sword[(sizeof(sword) == 2) * 2 - 1];
typedef int ICE_Dummy_uword[(sizeof(uword) == 2) * 2 - 1];
typedef int ICE_Dummy_udword[(sizeof(udword) == 4) * 2 - 1];
typedef int ICE_Dummy_sdword[(sizeof(sdword) == 4) * 2 - 1];
typedef int ICE_Dummy_uqword[(sizeof(uqword) == 8) * 2 - 1];
typedef int ICE_Dummy_sqword[(sizeof(sqword) == 8) * 2 - 1];
...
constexpr auto INVALID_ID = 0xffffffff;	//!<	Invalid dword ID (counterpart of null pointers);
constexpr auto INVALID_KID = 0xffff;	//!<	Invalid Kernel ID;
constexpr auto INVALID_NUMBER = 0xDEADBEEF;	//!<	Standard junk value;

constexpr auto LOCAL_EPSILON = 0.000001f;
...
template <class T>
inline const T& TMin(const T& a, const T& b) { return b < a ? b : a; }

template <class T>
inline const T& TMax(const T& a, const T& b) { return a < b ? b : a; }

template <class T>
inline void TSetMin(T& a, const T& b) { if (a > b) a = b; }

template <class T>
inline void TSetMax(T& a, const T& b) { if (a < b) a = b; }
