...
//! TO BE DOCUMENTED
ICF bool planeBoxOverlap(const icePoint& normal, const float d, const icePoint& maxbox)
{
	icePoint vmin, vmax;
...
public:
	...
	IC void _init(COLLIDER* CL, Fvector* V, TRI* T, const Fvector& C, const Fvector& E)
	{
		...
		verts = V;
		tris = T;
		center = icePoint(C.x, C.y, C.z);
		extents = icePoint(E.x, E.y, E.z);
		b_min.sub(C, E);
		b_max.add(C, E);
	}
