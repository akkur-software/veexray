 ...
 *	3D point.
 *
 *	The name is "icePoint" instead of "Vector" since a vector3 is N-dimensional, whereas a point is an implicit "vector3 of dimension 3".
...
//### could be optimized
icePoint& icePoint::Unfold(Plane& p, icePoint& a, icePoint& b)
{
	...
	/*	icePoint v = *this;
		...
		if(FastFabs(Angle)<0.0001f)	return *this;	// icePoint is already on plane // ## project
	
		// Rotate around (a,b)
		// 1) Move to origin
		icePoint p0t(0.0f, 0.0f, 0.0f);
		icePoint p1t = b - a;
		icePoint p2t = v - a;
	
		// 2) Rotate so that rotation axis = Z axis
		Matrix3x3 M;
		icePoint Axis = (p1t - p0t).Normalize();
		float DpX = FastFabs(Axis.x);
		float DpY = FastFabs(Axis.y);
		float DpZ = FastFabs(Axis.z);
				if(FastFabs((Axis|icePoint(0.0f, 0.0f, 1.0f)) - 1.0f) < 0.0001f)	M.Identity();
		else	if(FastFabs((Axis|icePoint(0.0f, 0.0f, 1.0f)) + 1.0f) < 0.0001f)	{ M.Identity(); Angle = -Angle; }
		else																	M.MapToZ(Axis);
